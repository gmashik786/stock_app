...
if option=="All institutions":
    ...
    temp1={"Price":s1}
    maxd=pd.DataFrame(temp1).style.background_gradient(cmap = 'copper')
    ...
    st.write("**The maximum Close price for each institution's stock for **",today.year)
    st.dataframe(maxd23)
    returns=pd.DataFrame()
    for name in l1:
    ...
    st.write("**Similar analysis for year",today.year," (Dark red is the riskiest one)**")
    st.dataframe(maxd2)
...
pr_option= st.selectbox('Select institution for which you want to predict the stock price:',('Select an institution','Bank of America', 'CitiGroup','Goldman Sachs', 'JPMorgan Chase','Morgan Stanley'))
preiod=st.slider('How many days you want to predict:',5,10,value=7)
